...
# 全局变量
DOMAIN=""
SOURCE_URL="https://d1vnr7amzbx49s.cloudfront.net/index_1.m3u8"  # 默认固定的源URL
LOCAL_PATH=""
USERNAME="admin"
PASSWORD="admin123"
CERT_DIR="/root/cert"
...
CONFIG_FILE="/etc/m3u8proxy/config.conf"

# 标题展示
show_banner() {
    clear
    echo -e "${BLUE}====================================================${NC}"
    echo -e "${GREEN}         M3U8 直播转发工具 v1.1                   ${NC}"
    echo -e "${BLUE}====================================================${NC}"
    echo ""
}
...
PASSWORD=$PASSWORD
LOCAL_DIR=$LOCAL_DIR
DOMAIN_CERT_DIR=$DOMAIN_CERT_DIR
SOURCE_HOST=$SOURCE_HOST
SOURCE_DIR=$SOURCE_DIR
EOL
    
    chmod 600 "$CONFIG_FILE" # 设置只有 root 用户可以读写
}
...
parse_all_inputs() {
    ...
    if [ "${#INPUTS[@]}" -lt 2 ]; then
        echo -e "${RED}错误：参数不足。需要至少提供域名和本地路径${NC}"
        return 1
    fi
...
collect_info() {
    ...
    echo -e "${YELLOW}格式: 域名 本地路径 [用户名] [密码]${NC}"
    echo -e "${YELLOW}例如: news.example.com /live/ admin my_password${NC}"
    echo -e "${YELLOW}源 URL 固定为: ${SOURCE_URL}${NC}"
...
EOL
    ...
    # 删除旧的续签任务（如果存在）
    (crontab -l 2>/dev/null || echo "") | grep -v "renew_cert_" > /tmp/crontab.tmp
    
    # 添加新的续签任务
    echo "15 3 * * * $RENEW_SCRIPT" >> /tmp/crontab.tmp
...
update_config() {
    ...
    # 提示用户当前配置已保存
    echo -e "${YELLOW}注意: 将保留源URL为 ${SOURCE_URL}${NC}"
...
main() {
    ...
    if load_config; then
        ...
        while true; do
            echo -e "\n${YELLOW}请选择操作:${NC}"
            echo -e "${BLUE}1)${NC} 使用新的域名和路径重新配置 (保留源 URL)"
            echo -e "${BLUE}2)${NC} 退出"
